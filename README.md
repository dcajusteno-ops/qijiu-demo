...
- 新增 Windows 安装程序，可在安装时选择目标目录
- 安装目录默认指向 `%USERPROFILE%\Comfy Manager`，同时允许用户手动修改
- 安装版运行时数据统一跟随安装目录，`data/` 与 `.trash/` 不再落到系统盘其他位置
- 安装包内置 `data/prompt-library/`，保证首次安装后即可正常使用提示词词库
- 同步更新安装说明、发布流程与项目文档，统一到 `v2.1.5`
