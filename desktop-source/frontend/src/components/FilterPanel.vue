  ...
  <Popover>
    <PopoverTrigger as-child>
      <Button variant="ghost" size="icon" title="高级筛选">
        <Filter class="h-4 w-4" :class="{ 'text-primary fill-primary/20': filters.dateRange.start || filters.dateRange.end || filters.size.min || filters.size.max || filters.dimensions.minW || filters.dimensions.minH }" />
      </Button>
    </PopoverTrigger>
    <PopoverContent class="w-80 p-4" align="start" side="bottom" :side-offset="4">
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" class="h-auto p-0 text-xs text-muted-foreground hover:text-foreground" @click="resetFilters">
            重置
          </Button>
        </div>
        ...
        <div class="space-y-2">
            ...
            <div class="flex items-center gap-2">
                <Input type="number" placeholder="Min" class="h-8 text-xs px-2" v-model.number="filters.size.min" />
                <span class="text-muted-foreground">-</span>
                <Input type="number" placeholder="Max" class="h-8 text-xs px-2" v-model.number="filters.size.max" />
            </div>
        </div>
        ...
        <div class="flex items-center justify-between space-x-2">
          <Label for="stacking-toggle" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            自动折叠相似图片
          </Label>
          <Switch
            id="stacking-toggle"
