...
<template>
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end" class="w-56">
            ...
            <!-- Manage button -->
            <DropdownMenuSeparator />
            <DropdownMenuItem @click="manageOpen = true" class="flex items-center gap-2 cursor-pointer">
        ...
        <DialogContent class="sm:max-w-[520px] max-h-[80vh] flex flex-col overflow-hidden p-6 select-none">
            ...
            <div class="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1">
                ...
                <div v-if="customLinks.length > 0" class="space-y-2">
                    <div v-for="link in customLinks" :key="link.id"
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button size="icon" variant="ghost" class="h-7 w-7" @click="editLink(link)" title="编辑">
                                <Edit2 class="w-3 h-3" />
                            </Button>
                            <Button size="icon" variant="ghost" class="h-7 w-7 text-destructive hover:text-destructive hover:bg-destructive/10" @click="requestDeleteLink(link)" title="删除">
                ...
                <div class="border rounded-md p-3 space-y-3 bg-muted/20">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="space-y-1">
                            <Label class="text-xs">图标文字 (可选, 2个字符)</Label>
